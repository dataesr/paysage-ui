...
export default function EmailTypesForm({ id, data, onSave, onDelete }) {
    ...
    <form>
      <Container fluid>
        ...
        <Row>
          ...
          <Col n="12" spacing="pb-3w">
            <TextInput
              label="Identifiant Annelis"
              value={form.annelisId || ''}
              onChange={(e) => updateForm({ annelisId: e.target.value })}
            />
          </Col>
        </Row>
        <FormFooter
...
EmailTypesForm.defaultProps = {
  data: { usualName: null, otherNames: [], annelisId: null },
  id: null,
  onDelete: null,
};
