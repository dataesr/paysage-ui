      ...
      <div className={`fr-card__body ${styles['card-body']} ${styles[`${toPrintRelation.collection}-border`]} ${((relation.current !== undefined) && !relation.current) && 'turngrey'}`}>
        <div className="fr-card__content">
          <p className="fr-card__desc">
            ...
            {relation?.resource.collection === 'prices' && inverse && (relation.startDate || relation.endDate) ? formatDescriptionDates(relation.startDate || null, relation.endDate || null).replace('depuis le', '') : formatDescriptionDates(relation.startDate || null, relation.endDate || null)}
          </p>
