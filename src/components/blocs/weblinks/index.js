...
export default function Weblinks({ types, title }) {
  ...
  const getElementsCount = () => {
    if (!data) return null;
    return types?.length
      ? data.data.filter((el) => types.includes(el.type))?.length
      : data.data.length;
  };
  ...
  const renderCards = () => {
    ...
    const list = filteredList.map((el) => (
      <div
        key={el.id}
        className="fr-card fr-card--sm fr-card--grey fr-card--no-border"
      >
